...
jQuery(document).ready(function() {
    ...
    jQuery('.open-map').click(function(){
        jQuery('.all-map-section .this-map').slideToggle(500);
    });
    ...
    jQuery(".booking-form").submit(function() {
        ...
        jQuery.ajax({
            type: "POST",
            url: "/dev/contact.php", // полный путь до файла PHP
            data: str,
            success: function(msg) {
                if(msg == 'OK') {
                    jQuery('.note').text('Ваш запрос принят, наши менеджеры свяжутся с Вами.');
                    jQuery(".booking-form").hide(500);
                } else {
                    jQuery('.note').text('При отправке письма возникла ошибка. Повторите запрос позже.');
                    jQuery(".booking-form").hide(500);
                }
            }
        });
    ...
    jQuery(".news-letter-form").submit(function() {
        var str = jQuery(this).serialize();

        jQuery.ajax({
            type: "POST",
            url: "/dev/sub.php", // полный путь до файла PHP
            data: str,
            success: function(msg) {
                if(msg == 'OK') {
                    jQuery('.news-letter-form input[type="submit"]').val('Спасибо, мы вам перезвоним');
                } else {
                    jQuery('.news-letter-form input[type="submit"]').val('Ошибка');
                }
            }
        });
        return false;
